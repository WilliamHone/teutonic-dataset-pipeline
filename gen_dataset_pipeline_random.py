...
def main():
    parser = argparse.ArgumentParser(
        description="Download random shards, extract sequences, and save to JSONL."
    )

    # Core parameters requested
    parser.add_argument(
        "--num_shards",
        type=int,
        required=True,
        help="Number of random shards to download and process",
    )
    parser.add_argument(
        "--num_samples",
        type=int,
        default=3000,
        help="Number of samples to extract per shard (default: 4000)",
    )
    ...
    # Determine how many shards to actually download
    num_to_download = min(args.num_shards, total_objects)
    if num_to_download < args.num_shards:
        print(
            f"⚠️ Requested {args.num_shards} shards, but only {total_objects} available. Downloading all {total_objects}."
        )

    print(f"Loaded {total_objects} objects from '{args.shards_json}'.")
    print(f"Randomly selected {num_to_download} shards to process.")

    # 2. Randomly choose the shards
    chosen_objects = random.sample(data, num_to_download)
    ...
    # 4. Sequentially process each shard (Download -> Extract -> Save -> Delete)
    for i, obj in enumerate(chosen_objects):
        print(f"\n--- Processing Shard [{i+1}/{num_to_download}] ---")
